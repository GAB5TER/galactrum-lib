...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new orecore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new orecore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = orecore.crypto.Hash.sha256sha256(Buffer.alloc(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new orecore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function orecoreTest() {
      block1 = orecore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('orecore', orecoreTest, {maxTime: maxTime});
